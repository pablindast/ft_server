...
RUN service mysql start &&  mysql -u root -e "CREATE USER 'mkhemakh'@'localhost' IDENTIFIED BY 'mkhemakh123';"
RUN	wget http://wordpress.org/latest.tar.gz && tar xzvf latest.tar.gz && mv wordpress /var/www/wordpress && chown -R www-data:www-data /var/www/wordpress && service mysql start &&  mysql -u root -padem123 -e  "CREATE DATABASE wordpress; GRANT ALL PRIVILEGES ON wordpress.* TO mkhemakh@localhost; FLUSH PRIVILEGES;"
